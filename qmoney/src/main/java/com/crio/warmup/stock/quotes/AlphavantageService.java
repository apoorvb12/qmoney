...
public class AlphavantageService implements StockQuotesService {
  ...
  public List<Candle> getStockQuote(String symbol, LocalDate from, LocalDate to)
    ...
    // TODO Auto-generated method stub
      ...
      try{
        dailyResponse  = restTemplate.getForObject(url, AlphavantageDailyResponse.class);
      ...
      // for(Map.Entry<LocalDate,AlphavantageCandle> map: dailyResponse.getCandles().entrySet()){
      //   LocalDate date = map.getKey();
      //   if(date.isAfter(from) && date.isBefore(to)){
      //     AlphavantageCandle candle = map.getValue();
      //     candle.setDate(date);
      //     list.add(candle);
      //   }
      // }
      // Collections.sort(list, new AlphavantageCandle());  
      // return list;
      
      for(LocalDate date = from.plusDays(1);!date.isAfter(to); date = date.plusDays(1)){
        AlphavantageCandle candle = dailyResponse.getCandles().get(date);
